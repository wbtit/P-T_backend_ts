  ...
  async handleCreateResponse(req: AuthenticateRequest, res: Response) {
    ...
    (async () => {
      try {
        ...
        if (rfi) {
          const eventType = parentRespId ? "RFI_REPLY_ADDED" : "RFI_RESPONSE_RECEIVED";
          await notifyProjectStakeholdersByRole(rfi.project_id, RFI_NOTIFY_ROLES, (role) =>
            buildRoleScopedNotification(role, {
              ...
              templates: {
                ...
                external: { title: parentRespId ? "RFI Reply Received" : "RFI Response Received", message: "A new RFI response was received for your action." },
                oversight: { title: parentRespId ? "RFI Reply Added" : "RFI Response Received", message: "A new RFI response was submitted and is available for review." },
                internal: { title: parentRespId ? "RFI Reply Added" : "RFI Response Received", message: "A new RFI response was submitted in the project." },
                default: { title: parentRespId ? "RFI Reply Added" : "RFI Response Received", message: "A new RFI response was submitted." },
              },
            }),
        ...
        console.error("Error in handleCreateResponse rfi background tasks:", error);
      }
    })();
