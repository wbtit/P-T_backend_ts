...
(async () => {
  ...
  const before: any[] = await prisma.$queryRawUnsafe(
    `SELECT count(*) FROM standard_documents WHERE id = ANY($1::uuid[])`, ids
  );
  console.log("standard_documents rows matching predicate BEFORE delete:", before);

  const deleted: number = await prisma.$executeRawUnsafe(
    `DELETE FROM standard_documents WHERE id = ANY($1::uuid[])`, ids
  );
  ...
  console.log("standard_documents rows matching predicate AFTER delete:", after);
